...
Class Action {
	...
	function save_folder(){
		...
		if(empty($id)){
			...
			$check = $this->db->query("SELECT * FROM folders where user_id ='".$_SESSION['login_id']."' and status  ='".$status."' and name  ='".$name."'")->num_rows;
			...
			$check = $this->db->query("SELECT * FROM folders where user_id ='".$_SESSION['login_id']."' and status  ='".$status."' and name  ='".$name."' and id !=".$id)->num_rows;
			if(isset($is_public) && $is_public == 'on')
						$data .= ", is_public = 1 ";
						else
						$data .= ", is_public = 0 ";
	...
	function save_files(){
		...
		if($_FILES['upload']['tmp_name'] != ''){
					...
					if($move){
						...
						$file = explode('.',$file);
						$chk = $this->db->query("SELECT * FROM files where SUBSTRING_INDEX(name,' ||',1) = '".$file[0]."' and status = '".$status."' and folder_id = '".$folder_id."' and file_type='".$file[1]."' ");
						if($chk->num_rows > 0){
	...
	function file_rename(){
		...
		$file[0] = $name;
		$file[1] = $type;
		$chk = $this->db->query("SELECT * FROM files where SUBSTRING_INDEX(name,' ||',1) = '".$file[0]."' and status = '".$status."' and folder_id = '".$folder_id."' and file_type='".$file[1]."' and id != ".$id);
		if($chk->num_rows > 0){
			$file[0] = $file[0] .' ||'.($chk->num_rows);
			}
